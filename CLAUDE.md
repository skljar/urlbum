...
- ✅ Настройки (Файл → Настройки): `src/settings.rs`, `settings.json` рядом с exe; вкладка "Общие" — 4 рабочих опции: показывать toolbar (if-обёртка в Slint), сворачивать другие ветки при открытии (`collapse_siblings_if_needed` в 4 местах), подтверждать удаление (диалог + `on_request_delete`), запретить повторяющиеся ссылки (`db::url_exists` при создании); каркас вкладок Прокси/Рисунок (заглушки)
